-- Add the original 17x10, 120-second Apple Ten ruleset to the authenticated
-- run service and private global leaderboard.

alter table public.leaderboard_scores
  drop constraint if exists leaderboard_scores_game_check;

alter table public.leaderboard_scores
  add constraint leaderboard_scores_game_check
  check (game in ('three-cushion', 'four-ball', 'yacht', 'apple'));
...
alter table public.leaderboard_scores
  add constraint score_range check (
    ...
    or (game = 'apple' and best_score between 0 and 170)
  );

alter table public.arcade_runs
  drop constraint if exists arcade_runs_game_check;

alter table public.arcade_runs
  add constraint arcade_runs_game_check
  check (game in ('three-cushion', 'four-ball', 'yacht', 'apple'));
...
as $$
  ...
  where scores.game = p_game
    and p_game in ('three-cushion', 'four-ball', 'yacht', 'apple')
...
begin
  ...
  if p_game not in ('three-cushion', 'four-ball', 'yacht', 'apple') then
...
begin
  ...
  if (p_game in ('three-cushion', 'four-ball') and p_score not between 0 and 200)
    or (p_game = 'yacht' and p_score not between 0 and 359)
    or (p_game = 'apple' and p_score not between 0 and 170) then
